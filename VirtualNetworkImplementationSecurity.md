...
1. Confirm you have a resource group **Challenge-<inject key="DeploymentID" enableCopy="false"/>** present as shown in the below screenshot. You need to use the **Challenge-<inject key="DeploymentID" enableCopy="false"/>** resource group throughout this challenge.
...
1. The virtual network named “primary-vnet” in the East US region should be divided into two subnets: one for the primary application and another for the primary database. The application subnet should have a CIDR block of 10.0.0.0/24, and the database subnet should have a CIDR block of 10.0.1.0/24. The VNET CIDR must support adding atleast 60 subnets in the future with /24 range.

1. The virtual network named “secondary-vnet” in the West US region should be divided into two subnets: one for the secondary application and another for the secondary database. The application subnet should have a CIDR block of 10.20.0.0/24, and the database subnet should have a CIDR block of 10.20.1.0/24. The VNET CIDR must support adding atleast 60 subnets in the future with /24 range. 

1. Virtual machines between the secondary database and primary database must be able to communicate with each other over private IPs without going through public IP addresses. 

1. Virtual machines in primary-vnet must not be allowing RDP 3389 ports from any source. You must implement an alternate method for RDP which does not require exposing RDP 3389 port, both internally and externally. 

1. The virtual machines in the primary-database subnet must be able to access a blob storage account named “appstorageSUFFIX” securely by the private network without going through the internet. 

     >Make sure to replace the “SUFFIX” in the stoarge account name with random digits.

1. The virtual network should have a network security group (NSG) associated with all subnets. The NSG should have any rules except than required for this setup.
